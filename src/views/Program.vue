<template>
    ...
    <div class="mt-6 flex justify-center w-screen sm:mb-6 lg:mb-0" alt="hero" v-if="show">
        <div class="border-red-500 border-2 border-solid sm:max-w-xl sm:w-xl lg:max-w-4xl lg:w-4xl p-4 rounded-md w-[100%] animatie">
            <a @click="getItem(`#${circuitName.replace(/\s/g, '').toLocaleLowerCase()}`)" style="text-decoration:none; color:black" class="">
    ...
    <div alt="card-grid" class="lg:grid lg:grid-cols-2 md:grid-cols-1 lg:px-14 lg:p-6 lg:gap-6 sm:justify-center sm:flex sm:flex-col sm:gap-6">
        <div class="border-black border-2 border-solid sm:max-w-xl lg:max-w-4xl p-4 sm:ml-20 md:ml-24 lg:ml-0 rounded-md animatie" v-for="(cursa ,index) in curse" :key="index" :id="cursa.Circuit.circuitId.replace(/_/g, '')" :class="{cursaCurenta: idCurent===index}">
            <div alt="header" class="flex justify-between text-2xl mb-4 font-bold">
            ...
            <div alt="data-wrapper" class="text-2xl gap-2 flex flex-col">
                <div alt="FP1" class="flex justify-between">
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.FirstPractice.date + "T" + cursa.FirstPractice.time).toLocaleTimeString('ro-RO',{ timeStyle: "short" }) + " " + "-" + " " + add1(cursa.FirstPractice.date + "T" + cursa.FirstPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
                <div alt="Quali" v-if="cursa.Sprint" class="flex justify-between">
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.Qualifying.date + "T" + cursa.Qualifying.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1(cursa.Qualifying.date + "T" + cursa.Qualifying.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
                <div alt="FP2" class="flex justify-between">
                    <div alt="parte-st" class="flex justify-between w-[50%]">
                        <p>FP2</p>
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.SecondPractice.date + "T" + cursa.SecondPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1(cursa.SecondPractice.date + "T" + cursa.SecondPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
                <div alt="Sprint" v-if="cursa.Sprint" class="flex justify-between">
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.Sprint.date + "T" + cursa.Sprint.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1(cursa.Sprint.date + "T" + cursa.Sprint.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
                <div alt="FP3" v-if="cursa.ThirdPractice" class="flex justify-between">
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.ThirdPractice.date + "T" + cursa.ThirdPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1(cursa.ThirdPractice.date + "T" + cursa.ThirdPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
                <div alt="Quali" v-if="!cursa.Sprint" class="flex justify-between">
                    <div alt="parte-st" class="flex justify-between w-[50%]">
                        <p>Quali</p>
                ...
                <div alt="Cursa" class="flex justify-between">
                    ...
                    <p alt="timp" class="w-[50%] flex justify-end items-center">{{new Date(cursa.date + "T" + cursa.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})}}</p>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        async getCurse () {
            ...
            this.FP1.time = new Date(this.hero.FirstPractice.date + "T" + this.hero.FirstPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1Hour(this.hero.FirstPractice.date + "T" + this.hero.FirstPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
            this.FP2.date = new Date(this.hero.SecondPractice.date).toISOString().replace(/T.*/,'').split('-').reverse().join('-')
            this.FP2.time = new Date(this.hero.SecondPractice.date + "T" + this.hero.SecondPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1Hour(this.hero.SecondPractice.date + "T" + this.hero.SecondPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
            if(this.hero.ThirdPractice){
                this.FP3.date = new Date(this.hero.ThirdPractice.date).toISOString().replace(/T.*/,'').split('-').reverse().join('-')
                this.FP3.time = new Date(this.hero.ThirdPractice.date + "T" + this.hero.ThirdPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1Hour(this.hero.ThirdPractice.date + "T" + this.hero.ThirdPractice.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
            }
            if(this.hero.Sprint){
                this.Sprint.date = new Date(this.hero.Sprint.date).toISOString().replace(/T.*/,'').split('-').reverse().join('-')
                this.Sprint.time = new Date(this.hero.Sprint.date + "T" + this.hero.Sprint.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1Hour(this.hero.Sprint.date + "T" + this.hero.Sprint.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
            }
            this.Quali.date = new Date(this.hero.Qualifying.date).toISOString().replace(/T.*/,'').split('-').reverse().join('-')
            this.Quali.time = new Date(this.hero.Qualifying.date + "T" + this.hero.Qualifying.time).toLocaleTimeString('ro-RO',{timeStyle: "short"}) + " " + "-" + " " + add1Hour(this.hero.Qualifying.date + "T" + this.hero.Qualifying.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
            this.Race.date = new Date(this.hero.date).toISOString().replace(/T.*/,'').split('-').reverse().join('-')
            this.Race.time = new Date(this.hero.date + "T" + this.hero.time).toLocaleTimeString('ro-RO',{timeStyle: "short"})
